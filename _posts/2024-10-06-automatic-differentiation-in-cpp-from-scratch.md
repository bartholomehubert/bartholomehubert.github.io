...
Automatic differentiation is an algorithm that makes the computation of these derivitives easy.
This article presents an implementation of automatic differentiation using C++20, using the [Eigen library](https://eigen.tuxfamily.org/index.php) for linear algebra operations.

In a follow-up [article](/2024-10-09), we will train a simple neural network, based on the implementation of this article.

# How computational graphs work
Computational graphs are at the core of automatic differentiation.
...
![computational graph](/assets/a2-1.png)
Computational graphs are widely used because they make easier the computation of derivatives by using automatic differentiation.

# Calculating the derivatives
When it comes to computing derivatives, we have several options. Let's examine why automatic differentiation stands out:
...
- Suffers from floating-point rounding errors
- Requires multiple graph evaluations, one for each input variable

A better alternative would be to use automatic differentiation, which doesn't have these three flaws.

## Automatic differentiation
Automatic differentiation offers two approaches: forward-mode and reverse-mode. Both provide exact derivatives (up to floating-point precision) but with different computational characteristics.
...
![backward-mode illustration](/assets/a2-2.png)
*Forward-mode differentiation when calculating $$\frac{dx_6}{dx_2}$$*
![backward-mode illustration](/assets/a2-3.png)
...
As an example:
![backward-mode illustration](/assets/a2-5.png)
...
These formulas comes from the application of the chain rule and understanding how each element contributes to the final result.

# The implementation
## Prerequisites
Before diving in, you should be familiar with:
- Basic C++ (templates, inheritance, polymorphism, smart pointers)
- The Eigen library (check out my [brief introduction to Eigen](/2024-10-05) or the [official documentation](https://eigen.tuxfamily.org/dox/group__DenseMatrixManipulation__chapter.html))
...
Note that the `backward` method adds the gradient coming from different paths, as evocated in the note previously.
...
```
{% endraw %}
This wrapper uses shared pointers to manage the lifetime of expressions, allowing nodes to persist even after their creating matrices go out of scope.
...
```
{% endraw %}
Here's how we implement binary operations like matrix addition and multiplication:
`expr.hpp`
{% raw %}
```cpp
/* ... */
...
```
{% endraw %}
The implementation of `SubstractExpr</var>, <var>DivideScalarExpr</var>, and <var>TransposeExpr` aren't shown here as they are really similar to the ones above.
...
```
{% endraw %}
This is a lot of boilerplate code, as they all return an expression of their respective operation. I have just included here the essential ones
...
1. Eliminate temporary objects using Eigen's expression templates (as every current operation creates a temporary)
1. Implement memory pooling for expression nodes
1. Add support for more operations and optimizations

In the [second part of this article](/2024-10-09), we'll use this implementation to train a neural network!

## Conclusion
While our implementation priotizes clarity over performance, it is a fully-working demonstration of the automatic differentiation algorithm.